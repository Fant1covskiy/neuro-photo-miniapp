    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50" style={{ paddingBottom: '140px' }}>
      ...
      <div className="px-4 py-6">
        <div className="space-y-3">
          {cart.map((item, index) => (
            <div key={item.id} className="bg-white rounded-2xl shadow-md overflow-hidden flex">
              ...
              <div className="flex-1 p-4 flex flex-col justify-between">
                <div>
                  <h3 className="font-bold text-base mb-1 text-gray-800 line-clamp-1">{item.name}</h3>
                  {item.description && <p className="text-gray-500 text-xs line-clamp-1">{item.description}</p>}
                </div>

                <div className="flex items-center justify-between">
                  <span className="text-purple-600 font-bold text-xl">{Number(item.price || 0).toFixed(2)} ₽</span>
                  <button onClick={() => removeFromCart(item.id)} className="p-2 hover:bg-red-50 rounded-lg transition-colors">
